...
class Adam:
    ...
    def update_params(self, layer: Linear):
        ...
        layer.weights += -self.learning_rate * bias_corrected_mt_w / (np.sqrt(bias_corrected_vt_w + self.epsilon))

        layer.biases += -self.learning_rate * bias_corrected_mt_b / (np.sqrt(bias_corrected_vt_b + self.epsilon))
